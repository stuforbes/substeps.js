module.exports = function StepRegistry(_){
  ...
  callProcessorListItem = function(type, idx, executionState, callback){
    if(idx < processors[type].length){
      var processor = processors[type][idx];
      processor(executionState, function(error){
        if(!error){
          callProcessorListItem(type, idx+1, executionState, callback);
        }
      })
    } else{
      if(callback){
