    ...
    scenarioExecutor: function (scenario) {
      return function () {
        ...
        callbackIterator.iterateOver(scenario.steps, function (step, callback) {
          ...
          var params;
          if(step.definition){
            params = parameterExtractor.extractFor(step.text, step.definition.pattern, step.definition.parameters);
          } else if(step.stepImpl){
            params = parameterExtractor.extractFor(step.text, step.stepImpl.pattern, step.stepImpl.parameters);
          } else {
            params = [];
          }
          step.execute(params, callback);
        });
