module.exports = function FileToFeature(directives, stringTools, _) {
  ...
  var processExampleRow = function (text, previous, failure) {
    if(text.charAt(0) !== '|' || text.charAt(text.length-1) !== '|'){
      failure.isFailure = true;
      failure.message = 'Values in an example row must be separated by the \'|\' character, including at the start and end of the row';
    } else{
  ...
  var isStepContainerDirective = function(previous){
    return previous.node && previous.node.steps;
  };
        ...
        if (!failure.isFailure) {
          ...
          if (trimmed.length > 0) {
            ...
            if (directive !== undefined) {
              ...
              processExampleRow(trimmed, previous, failure);
            } else if(isStepContainerDirective(previous)){
              resetScopedTags(scopedTags);
              processStep(trimmed, previous, feature, lineNumber, failure);
            } else if(previous.type === 'feature'){
              // probably not the best way to do this - sometimes a feature is declared with a long description that spans
              // multiple lines. This could also be true of other directives (e.g. background, scenario etc). Need to think
              // of a good way of handling this completely
              previous.node.feature += '\n'+trimmed
            }
          }
