module.exports = function FileToFeature(directives, stringTools, _) {
  ...
  var processExampleRow = function (text, previous, feature) {
  ...
  var processStep = function (text, previous, feature, lineNumber, failure) {
    if(previous.node.steps){
      previous.node.steps.push({step: text});
    } else{
      failure.isFailure = true;
      failure.message = 'Could not create feature. Tried to add a step to directive ('+previous.type+') on line '+lineNumber
    }
  }
        ...
        if (!failure.isFailure) {
          ...
          if (trimmed.length > 0) {
            ...
            if (directive !== undefined) {
              ...
              processExampleRow(trimmed, previous, feature);
            } else {
