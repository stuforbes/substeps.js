...
//    var regex = /single/g;
  ...
  var extractParamsFrom = function(text){
    ...
    if(!params || params === null){
      return {pattern: new RegExp(text, 'g'), parameters: []};
    } else{
      var processedText = text;
      params.forEach(function(param){ processedText = processedText.replace(param, matchAllRegexText); });
      return {pattern: new RegExp(processedText, 'g'), parameters: _.map(params, function(p){ return p.substring(1, p.length-1); })};
    }
  };
